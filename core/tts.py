...
from core.utils import to_speakable
...
def speak(text: str, user_text_for_name_detect: str = None):
    print(f"[DEBUG-TTS-CALL] Speak requested: {repr(text)}")
...
class PiperEngine:
    ...
    def _stderr_tap(self, proc):
        """Reads Piper stderr lines and prints them for debugging."""
        try:
    ...
    def start(self, model_path=None, config_path=None):
        ...
        # Debug info for missing components
    ...
    def _read_loop(self):
        """Continuously read Piper audio output."""
        try:
